...
void pushdown(tp x, tp l, tp r) {
  tp mid = l + r >> 1;
  rec[x << 1] = mid - l + 1 - rec[x << 1];
  delay[x << 1] ^= 1;
  rec[x << 1 | 1] = r - mid - rec[x << 1 | 1];
  delay[x << 1 | 1] ^= 1;
  delay[x] ^= 1;
}

void modify(tp x, tp Lm, tp Rm, tp l, tp r) {
  if (r < Lm || Rm < l) {
    return;
  }
  if (l <= Lm && r >= Rm) {
  ...
  tp mid = Lm + Rm >> 1;
  modify(x << 1, Lm, mid, l, r);
  modify(x << 1 | 1, mid + 1, Rm, l, r);
  rec[x] = rec[x << 1] + rec[x << 1 | 1];
}
