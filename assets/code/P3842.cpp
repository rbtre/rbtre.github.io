...
tp ff[Hat_N], bf[Hat_N], l[Hat_N], r[Hat_N];
...
signed main() {
  ...
  ff[1] = loc(r[1], 1) + loc(r[1], l[1]);
  bf[1] = loc(r[1], 1);
  for (tp i = 2; i <= n; ++i) {
    ff[i] = min(ff[i - 1] + loc(l[i - 1], r[i]) + loc(r[i], l[i]),
                bf[i - 1] + loc(r[i - 1], r[i]) + loc(r[i], l[i])) +
            1;
    bf[i] = min(ff[i - 1] + loc(l[i - 1], l[i]) + loc(l[i], r[i]),
                bf[i - 1] + loc(r[i - 1], l[i]) + loc(l[i], r[i])) +
            1;
  }
  bout(min(ff[n] + loc(l[n], n), bf[n] + loc(r[n], n)));
  return 0;
}
...
void Put_Unsigned(Typex _Val) {
  char* _Stack = (char*)malloc(sizeof(Typex));
  unsigned S_top = 0;
  while (_Val) {
