...
signed main() {
  ...
  string s;
  cin >> s;
  low = count_if(s.begin(), s.end(), [](auto&& x) { return x == tolower(x); });
  high = count_if(s.begin(), s.end(), [](auto&& x) { return x == toupper(x); });
  for_each(s.begin(), s.end(), [&](auto&& x) { x = high > low ? toupper(x) : tolower(x); });
  cout << s << '\n';
  return 0;
}
