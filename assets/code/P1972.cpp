...
struct Rs {
  ...
  bool operator<(const Rs& comp) const { return r < comp.r; }
} ac[Hat_N];

tp rec[Hat_N], val[Hat_N], vis[Hat_N], res[Hat_N];
tp n;
...
signed main() {
  ...
  stable_sort(ac + 1, ac + m + 1);
  for (tp i = 1; i <= m; ++i) {
    for (tp j = next; j <= ac[i].r; ++j) {
      if (vis[val[j]]) {
        modify(vis[val[j]], -1);
      }
      modify(j, 1);
    ...
    next = ac[i].r + 1;   
    res[ac[i].id] = query(ac[i].r) - query(ac[i].l - 1);
  }
  for (tp i = 1; i <= m; ++i) {
    cout.endl(res[i]);
  }
