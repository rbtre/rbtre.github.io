...
constexpr tp Hat_N = 21, Hat_2N = 1 << Hat_N;

tp f[Hat_2N];
double locx[Hat_N], locy[Hat_N];

void Solve() {
  memset(f, 0b1111111, sizeof f);
  *f = 0;
  list<tp> state;
  tp n, m;
  bin(n, m);
  ...
  for (tp i(0); i ^ n; ++i) {
    for (tp j(i + 1); j < n; ++j) {
      if (locx[i] != locx[j]) {
        if (double x = (locy[i] - locy[j] * locx[i] / locx[j]) / locx[i] /
                       (locx[i] - locx[j]),
            y = (locy[i] * locx[j] * locx[j] / locx[i] / locx[i] - locy[j]) /
                (locx[j] * locx[j] / locx[i] - locx[j]);
            x < 0) {
          state.push_back(0);
          for (tp k(0); k ^ n; ++k) {
            if (abs(x * locx[k] * locx[k] + y * locx[k] - locy[k]) < 1e-8) {
              state.back() |= 1ll << k;
            }
          }
        }
  ...
  for (tp i(0), Endi = 1ll << n; i ^ Endi; ++i) {
    for (auto&& j : state) {
      if (f[i] + 1 < f[i | j]) {
        f[i | j] = f[i] + 1;
      }
