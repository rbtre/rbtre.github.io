...
constexpr tp Hat_S = 11, Hat_2S = 1 << Hat_S, Hat_D = 1003;

bool vis[Hat_S];
tp f[Hat_2S][Hat_D], val[Hat_S];
...
signed main() {
  ...
  while (t--) {
    ...
    **f = 1;
    ...
    for (tp U = 0, End_U = (1ll << n) - 1; U ^ End_U; ++U) {
      memset(vis, 0, sizeof vis);
      for (tp i = 1; i <= n; ++i) {
        if (!(U & (1ll << (i - 1))) && !vis[val[i]]) {
          vis[val[i]] = 1;
          for (tp k = 0; k < d; ++k) {
            f[U | (1ll << (i - 1))][(k * 10 + val[i]) % d] += f[U][k];
          }
        }
      }
    }
