...
void spfa(tp s) {
  ...
  dist[s] = 0;
  vis[s] = 1;
  for (SLF.push_back(s); SLF.size(); SLF.pop_front()) {
    ...
    vis[SLF.front()] = 0;
    LLL -= dist[SLF.front()];
    for (auto&& i : e[SLF.front()]) {
      if (dist[i.first] > dist[SLF.front()] + i.second) {
        dist[i.first] = dist[SLF.front()] + i.second;
        if (!vis[i.first]) {
          vis[i.first] = 1;
          LLL += dist[i.first];
          if (SLF.size() && dist[i.first] > dist[SLF.front()]) {
